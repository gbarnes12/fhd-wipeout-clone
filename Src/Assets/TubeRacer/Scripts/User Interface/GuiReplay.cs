...
public class GuiReplay : MonoBehaviour {
	...
	void OnGUI () 
	{
		...
		{
			SetGuiSkin ();
			Rect menuContent = DrawBackground (0.3f, false);
			DrawMenuButtons (menuContent,0.8f,300.0f, 20.0f);
			
			if(this.Enable3D)
			{
				Rect menuContent3D = DrawBackground (0.3f, this.Enable3D);
				DrawMenuButtons (menuContent3D,0.8f,300.0f, 20.0f);
			}
	...
	private Rect DrawBackground(float faktWidth, bool enabled3D){
		...
		if (!enabled3D)
			...
			GUI.DrawTexture (new Rect (Screen.width * 0.5f - backgrW + _positionGuiX, backgrFreeVSpace, backgrW, backgrH), _texMenuBackground, ScaleMode.ScaleToFit, true, 0.0f);
		...
		if(!enabled3D)
			...
			contentRect.x = Screen.width * 0.5f - backgrW+ _positionGuiX;
